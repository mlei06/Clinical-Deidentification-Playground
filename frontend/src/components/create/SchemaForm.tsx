...
export default function SchemaForm({ schema, formData, onChange, formContext }: SchemaFormProps) {
  ...
  const uiSchema = useMemo(() => schemaToUiSchema(normalizedSchema), [normalizedSchema]);
  ...
  return (
    <div className="schema-form text-sm [&_.field-description]:text-xs [&_.field-description]:text-gray-400 [&_input]:rounded [&_input]:border [&_input]:border-gray-300 [&_input]:px-2 [&_input]:py-1 [&_input]:text-sm [&_label]:mb-1 [&_label]:block [&_label]:text-xs [&_label]:font-medium [&_label]:text-gray-600 [&_select]:rounded [&_select]:border [&_select]:border-gray-300 [&_select]:px-2 [&_select]:py-1 [&_select]:text-sm [&_.form-group]:mb-3">
      <Form
