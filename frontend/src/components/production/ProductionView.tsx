...
export default function ProductionView() {
  ...
  const { data: health } = useDeployHealth();
  ...
  useEffect(() => {
    if (!mode && config?.default_mode) {
      const isAvailable =
        !health ||
        health.modes.find((m) => m.name === config.default_mode)?.available !== false;
      if (isAvailable) setMode(config.default_mode);
    }
  }, [config, health, mode, setMode]);

  const pipelineName = useMemo(() => {
    if (!mode) return null;
    if (health) {
      const h = health.modes.find((m) => m.name === mode);
      if (h && !h.available) return null;
      return h?.pipeline ?? null;
    }
    return config?.modes[mode]?.pipeline ?? null;
  }, [mode, health, config]);
